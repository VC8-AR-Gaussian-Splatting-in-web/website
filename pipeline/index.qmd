...
<style>
  ...
  .gs-card video{width:100%;display:block;aspect-ratio:1/1;object-fit:cover;background:#0d0d14;}
...
![Gartenhütte — die feinen Blätter & Schatten werden als Splats rekonstruiert](img/compare-gartenhuette.jpg)
...
| --- | --- | --- |
| `--strategy` | `default` (densifiziert frei) | `mcmc` (deckelt Gaussian-Zahl) |
| `--cap-max` | – | `1.000.000` |
| `--data-factor` | `1` (volle Auflösung) | `2` (halbe – spart VRAM) |
| `--sh-degree` | `3` | `3` |

`mcmc` deckelt die Anzahl der Gaussians → planbarer VRAM und weniger Floater bei großen,
ungleichmäßig abgedeckten Szenen.
:::
...
(function(){
  ...
  function start(){
    ...
    BABYLON.SceneLoader.ImportMeshAsync(null,"","../web-ar/models/vase.spz",scene)
      .then(function(res){
        var m=res.meshes[0];
        m.rotationQuaternion=BABYLON.Quaternion.RotationAxis(BABYLON.Axis.X,-Math.PI/2); // Vase ist Z-up -> Y-up
